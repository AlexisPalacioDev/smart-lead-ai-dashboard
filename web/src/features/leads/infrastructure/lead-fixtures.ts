import type { Lead } from "../domain/lead";

/**
 * lead-fixtures.ts
 * Provides deterministic seed data for tests, demos, and browser-only storage.
 * Assumes records remain stable so assertions and visual references do not drift.
 */

export const leadFixtures: Lead[] = [
  {
    id: "lead-1",
    name: "Ana Torres",
    email: "ana@example.com",
    phone: "3001001001",
    source: "instagram",
    productInterest: "CRM Starter",
    budget: 1200,
    createdAt: "2026-04-17T10:00:00.000Z",
  },
  {
    id: "lead-2",
    name: "Bruno Diaz",
    email: "bruno@example.com",
    phone: "3001001002",
    source: "facebook",
    productInterest: "CRM Pro",
    budget: 2500,
    createdAt: "2026-04-16T11:30:00.000Z",
  },
  {
    id: "lead-3",
    name: "Carla Ruiz",
    email: "carla@example.com",
    phone: "3001001003",
    source: "landing_page",
    productInterest: "AI Assist",
    budget: 1800,
    createdAt: "2026-04-15T09:15:00.000Z",
  },
  {
    id: "lead-4",
    name: "Diego Leon",
    email: "diego@example.com",
    phone: "3001001004",
    source: "referido",
    productInterest: "CRM Starter",
    budget: 800,
    createdAt: "2026-04-14T16:20:00.000Z",
  },
  {
    id: "lead-5",
    name: "Elena Mora",
    email: "elena@example.com",
    phone: "3001001005",
    source: "instagram",
    productInterest: "AI Assist",
    budget: 3200,
    createdAt: "2026-04-12T14:40:00.000Z",
  },
  {
    id: "lead-6",
    name: "Felipe Nunez",
    email: "felipe@example.com",
    phone: "3001001006",
    source: "otro",
    productInterest: "CRM Pro",
    budget: null,
    createdAt: "2026-04-10T08:05:00.000Z",
  },
  {
    id: "lead-7",
    name: "Gabriela Soto",
    email: "gabriela@example.com",
    phone: "3001001007",
    source: "facebook",
    productInterest: "CRM Starter",
    budget: 950,
    createdAt: "2026-04-08T12:10:00.000Z",
  },
  {
    id: "lead-8",
    name: "Hugo Perez",
    email: "hugo@example.com",
    phone: "3001001008",
    source: "landing_page",
    productInterest: "AI Assist",
    budget: 4100,
    createdAt: "2026-04-07T18:45:00.000Z",
  },
  {
    id: "lead-9",
    name: "Ines Vidal",
    email: "ines@example.com",
    phone: "3001001009",
    source: "referido",
    productInterest: "CRM Pro",
    budget: 2100,
    createdAt: "2026-04-03T13:00:00.000Z",
  },
  {
    id: "lead-10",
    name: "Julian Castro",
    email: "julian@example.com",
    phone: "3001001010",
    source: "instagram",
    productInterest: "CRM Starter",
    budget: 1500,
    createdAt: "2026-04-01T07:50:00.000Z",
  },
];
