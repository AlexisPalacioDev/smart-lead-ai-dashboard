 ...
 */
export function buildLeadStats(leads: Lead[], referenceDate: string): LeadStats {
  ...
  return {
    ...
    leadsLast7Days: countLeadsInLastDays(leads, referenceDate, DAYS_IN_WEEK_WINDOW),
    averageBudget:
      budgets.length === 0
        ? 0
        : Math.round(budgets.reduce((total, budget) => total + budget, 0) / budgets.length),
 ...
 */
function countLeadsInLastDays(leads: Lead[], referenceDate: string, days: number) {
...
function buildLeadsBySource(leads: Lead[]): Record<LeadSource, number> {
  return leads.reduce<Record<LeadSource, number>>(
    (sourceTotals, lead) => ({
      ...sourceTotals,
      [lead.source]: sourceTotals[lead.source] + 1,
    }),
    {
      instagram: 0,
      facebook: 0,
 ...
 */
function selectTopSource(leadsBySource: Record<LeadSource, number>): LeadSource {
  return (
    Object.entries(leadsBySource).sort((currentEntry, nextEntry) => {
      return nextEntry[1] - currentEntry[1]
    })[0]?.[0] as LeadSource | undefined
  ) ?? DEFAULT_TOP_SOURCE
}
